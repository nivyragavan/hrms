...
class OnBoardingChecklistController extends GetxController {
  static OnBoardingChecklistController get to =>
      Get.put(OnBoardingChecklistController());
  ...
  checklistAssign(int index) async {
    ...
    var response = GetStorage().read("RequestEncryption") == true
        ...
        encryptMessage: await LibSodiumAlgorithm().encryptionMessage(body),
        auth: true)
        : await _http.post(Api.checklistAssign, body,
        auth: true, contentHeader: false);
    if (response['code'] == "200") {
      UtilService()
  ...
  userTaskAnswer({List<
    ...
    print('userTaskAnswer body ${jsonEncode(body)}');
    var response = GetStorage().read("RequestEncryption") == true
        ? await _http.multipartPostData(
